 ...
 *
 * ПОЧЕМУ ИМЕННО ТАК:
 * · под плёнкой — НАСТОЯЩЕЕ фото стенда, а не собранная кладка. Через это прошли
 *   две неудачные версии: одна текстура с фильтрами (повтор зерна и цвета, которых
 *   у товара нет) и одиннадцать вырезанных кирпичей (плоские плашки без светотени —
 *   я отбирал участки по минимальному разбросу яркости и получил ровные заливки).
 *   Синтетическая кладка мультяшна по природе: у неё нет перспективы, неровных
 *   кромок и разной зернистости. Фото даёт всё это даром;
...
(function () {
  ...
  /* --- плёнка поверх настоящего фото стенда ------------------------------- */
  function build() {
